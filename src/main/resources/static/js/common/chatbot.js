...
function chatConnect() {
	...
	client.connect({}, (frame) => {
		key = generateUniqueKey(); //고유 키 생성
		console.log(key);
		client.subscribe(`/topic/answer/${key}`, (answer) => {
			// 메시지 처리를 별도의 비동기 함수로 분리
			handleMessage(answer);
		});
	});
}

// 메시지 처리를 위한 별도의 비동기 함수
async function handleMessage(answer) {
	var response = answer.body; //서버로부터 받은 메세지 객체
	var now = new Date();
	var time = formatTime(now);
...
async function messageLoading() {
	...
	// chatContent가 존재하는지 확인 후 사용
	if (!chatContent) {
		console.error("채팅 콘텐츠 요소를 찾을 수 없습니다");
		return;
	}

	// 로딩 메시지 요소 생성
	var loadingMessage = document.createElement('div');
	loadingMessage.classList.add('msg', 'bot', 'loading', 'new', 'flex');
	...
	avatarImg.src = '/img/chatbot-img.png';
	...
	// 봇 이름 생성
	var botNameDiv = document.createElement('div');
	botNameDiv.classList.add('bot-name');
	botNameDiv.textContent = '두비';

	// 채팅 파트 생성
	var chatPartDiv = document.createElement('div');
	chatPartDiv.classList.add('part', 'chat-chatbot');
	...
	// 로딩 애니메이션 컨테이너에 중간 점 추가
	loadingAnimationContainer.appendChild(middleDot);

	// 요소들 조립
	chatPartDiv.appendChild(loadingAnimationContainer);
	messageDiv.appendChild(botNameDiv);
	messageDiv.appendChild(chatPartDiv);
	loadingMessage.appendChild(iconDiv);
